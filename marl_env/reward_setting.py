...
class NoDealPenaltyReward(RewardSetting):
    ...
    def buyer_reward(self, buyer_deals):
        ...
        no_deal_penalty = - (1 + torch.max(torch.Tensor([self.env.market.time - self.no_deal_max, -1])))
        no_deal_penalty = torch.mul(no_deal_penalty, ~done_buyers)

        rew = torch.mul(self.env.b_reservations - buyer_deals, self.env.newly_finished_buyers)
